...
function fmt(intbuf, buf, wid, prec, widPresent, precPresent, minus, plus, sharp, space, unicode, uniQuote, zero) {
    this.intbuf = intbuf;
    this.buf = buf;
    this.wid = wid;
    this.prec = prec;
...
fmt.prototype.pad = function(b) {
    ...
    AssignStmt missing
    case :{
        [padding left right] 2126: = [0xc20008c700]
    };
    if (left > 0) {
...
fmt.prototype.padString = function(s) {
    ...
    AssignStmt missing
    case :{
        [padding left right] 2500: = [0xc20008cc80]
    };
    if (left > 0) {
        this.writePadding(left, padding);
    }
